            ...
            <div className="container mx-auto px-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-5xl mx-auto">
                    {accessLinks.map((link, index) => (
                        <div key={index} className="bg-white p-8 rounded-xl shadow-lg border border-slate-200 flex flex-col">
                            <div className={`rounded-full w-16 h-16 flex items-center justify-center mb-6 ${index === 0 ? 'bg-blue-100' : index === 1 ? 'bg-green-100' : 'bg-purple-100'}`}>{link.icon}</div>
                            ...
                            <Link href={link.href} className="font-semibold text-indigo-600 hover:text-indigo-800 transition-colors">{link.linkText} &rarr;</Link>
                        </div>
                    ))}
                </div>
            </div>
