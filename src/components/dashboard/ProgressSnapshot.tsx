    ...
    return (
        <div className="relative" style={{ width: size, height: size }}>
            <svg className="w-full h-full" viewBox={`0 0 ${size} ${size}`}>
                <circle className="text-slate-200 dark:text-slate-700" strokeWidth="8" stroke="currentColor" fill="transparent" r={radius} cx={size/2} cy={size/2} />
                <circle
            ...
            <span className="absolute inset-0 flex items-center justify-center text-xl font-bold text-slate-700 dark:text-slate-300">{Math.round(percentage)}%</span>
        </div>
    );
};
...
export const ProgressSnapshot = ({ initialCourseData }: { initialCourseData: Subject[] | null }) => {
    ...
    if (loading || !initialCourseData) {
         return (
             <div className="bg-white dark:bg-slate-800 p-6 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm animate-pulse h-[144px]">
                <div className="h-full bg-slate-100 dark:bg-slate-700 rounded"></div>
             </div>
        )
    }

    return (
        <div className="bg-white dark:bg-slate-800 p-6 rounded-lg border border-slate-200 dark:border-slate-700 shadow-md">
             {/* FIX: Added Header with Title and Link */}
             ...
             <div className="flex flex-col md:flex-row items-center gap-8">
                <div className="flex items-center gap-4 pr-8 border-b md:border-b-0 md:border-r border-slate-200 dark:border-slate-700 pb-6 md:pb-0 w-full md:w-auto">
                    ...
                    <div className="text-left">
                        ...
                        <p className="text-3xl font-bold text-slate-800 dark:text-slate-100">{completedLectures.size} / {totalLectures}</p>
                    </div>
                </div>
                <div className="w-full flex-grow grid grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-4">
                    {subjectStats.map(stat => {
                        ...
                        return (
                            <div key={stat.name}>
                                <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">{stat.name}</p>
                                <p className="text-lg font-bold text-slate-800 dark:text-slate-100">{stat.completed} / {stat.total} <span className="text-xs text-slate-500 dark:text-slate-400 font-normal">completed</span></p>
